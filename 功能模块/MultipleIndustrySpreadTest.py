...
from config_loader import traget_factor, holding_period, layers, test_window_start, test_window_end
...
def data_preprocessing(traget_factor,test_window_start,test_window_end):
    ...
    cv_df = pd.read_csv(r'C:\Users\63585\Desktop\PycharmProjects\pythonProject\QuantSystem\回测数据集\20170930-20251231_pipe.csv',
                        usecols=['trade_date', 'ts_code', 'circ_mv'])
    df_merge = df_loading.merge(cv_df,on=['trade_date','ts_code'],how='left')
    # 计算对数收益率与行业分类
    def calculate_lndret_exchange_identity(orginal_data, holding_period):
        # 对数收益率计算
    ...
    df, Stock_industry = calculate_lndret_exchange_identity(df_merge, holding_period)
    df_preprocess = cut_time_window(df, test_window_start, test_window_end)
    return df_preprocess, Stock_industry
# ─────────────────────────────────────────────────────────────────────────────
# 市值中性化处理
# ─────────────────────────────────────────────────────────────────────────────
def neutralize_factor_by_date(group, factor_col='raw_factor', cap_col='circ_mv'):
