...
'''
原理与15因子类似，捕捉量价共振/背离后的均值回归
Alpha_16 = -1 * rank(covariance(rank(high),rank(volumn),5))
'''
...
def calculate_alpha_16(data) -> pd.DataFrame:
    data_c = data.copy()
    ...
    data_c['cov_rank_high_rank_vol_5'] = data_c.groupby('ts_code').apply(
        lambda x: x['rank_high'].rolling(window=5).cov(x['rank_vol'])
    ).droplevel(level=0).sort_index()
    data_c['rank_cov_h_v'] = data_c.groupby('trade_date')['cov_rank_high_rank_vol_5'].rank(pct=True)
    data_c = data_c.dropna(subset=['rank_cov_h_v'])
    data_c['alpha_16'] = -data_c['rank_cov_h_v']

    data_c = remove_resume_window_data(data_c, window=5)  # 删除复牌日后窗口期内的异常因子值

    return_columns = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'industry_name', 'alpha_16']
    return_data = data_c[return_columns]
    return return_data

if __name__ == "__main__":
    print("--- 开始执行 Alpha#16 计算流程 ---")
    try:
        columns_needed = [
            'ts_code',
            'trade_date',
        ...
        processed_data = calculate_alpha_16(history_data)
        print(processed_data.head(3))

        print(f"Alpha#16 特征计算完成，共 {len(processed_data)} 行。")
        print(f"正在生成特征分布直方图。")
        distribution_plot(processed_data)
        print("正在保存数据...")
        save_data(processed_data, "alpha_16.csv")
        print("数据已保存。")
