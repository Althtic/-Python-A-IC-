...
def calculate_alpha_33(data) -> pd.DataFrame:
    start_time = time.time()
    data_c = data.copy()
    data_c = data_c.sort_values(by=['ts_code', 'trade_date'], ascending=[True,True]).reset_index(drop=True)

    data_c['alpha_33'] = -(1 - data_c['open'] / data_c['close'].replace(0, np.nan))

    # threshold = 0.3 
    # data_c['alpha_33'] = data_c['alpha_33'].clip(-threshold, threshold)

    data_c.loc[(data_c['open'] <= 0) | (data_c['close'] <= 0), 'alpha_33'] = np.nan
    # data_c['alpha_33'] = data_c.groupby('trade_date')['temp'].rank(pct=True)
    ...
    def mad_winsorize(x):
        ...
        if mad == 0: 
            return x
        bound = 5 * mad * 1.4826
        return x.clip(median - bound, median + bound)
    data_c['alpha_33'] = data_c.groupby('trade_date')['alpha_33'].transform(mad_winsorize)
    ...
    return_columns = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'industry_name', 'raw_factor', 'alpha_33']
...
if __name__ == "__main__":
    logger.info("--- 开始执行 Alpha#33 计算流程 ---")
    try:
        columns_needed = [
            'ts_code',
            'trade_date',
