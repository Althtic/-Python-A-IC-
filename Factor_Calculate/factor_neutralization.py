...
def neutralize_factor(df,
    ...
    # Empty windows are valid during sparse-history backfills. Return a
    # schema-preserving frame instead of an ambiguous groupby.apply result.
    if df is None or df.empty:
        result = df.copy() if isinstance(df, pd.DataFrame) else pd.DataFrame()
        if target_factor in result.columns:
            result = result.rename(columns={target_factor: 'raw_factor'})
            result[target_factor] = pd.Series(index=result.index, dtype='float64')
        elif target_factor:
            result['raw_factor'] = pd.Series(index=result.index, dtype='float64')
            result[target_factor] = pd.Series(index=result.index, dtype='float64')
    ...
    def process_single_date(group):
        ...
        industry_series = data_valid[industry_col].fillna('未分类')
    ...
    result_df = work_df.groupby(date_col, group_keys=False).apply(process_single_date)
