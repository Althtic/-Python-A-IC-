...
'''     
波动率反转因子
Alpha_34 = rank(((1-rank((std(returns,2)/std(returns,5))))+(1-rank(delta(close,1)))))
结合短期和长期市场信号的综合因子
'''
# --- 配置日志 ---
...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    # 波动率比率部分
    data_c['vol_ratio'] = data_c.groupby('ts_code')['dret'].transform(
        lambda x: x.rolling(window=2).std() / (x.rolling(window=5).std() + 1e-9) # 防止除0
    )
    data_c['partA'] = 1 - data_c.groupby('trade_date')['vol_ratio'].rank(pct=True)
    # 价格变化部分
    data_c['delta_close1'] = data_c['close'] - data['pre_close']
    data_c['partB'] = 1 - data_c.groupby('trade_date')['delta_close1'].rank(pct=True)
    # 最终因子部分
    data_c['alpha'] = -(data_c['partA'] + data_c['partB'])
    data_c['alpha_34'] = data_c.groupby('trade_date')['alpha'].rank(pct=True)
    ...
    return_columns = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'industry_name', 'raw_factor', 'alpha_34']
...
if __name__ == "__main__":
    logger.info("--- 开始执行 Alpha#34 计算流程 ---")
    try:
        columns_needed = [
            'ts_code',
            'trade_date',
        ...
        logger.info("正在保存数据...")
        save_data(processed_data, "alpha_34.csv")
        logger.info("数据已保存。")
    except Exception as e:
