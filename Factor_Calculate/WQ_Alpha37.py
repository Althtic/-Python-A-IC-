...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    data_c['delay1_oc_diff'] = data_c.groupby('ts_code')['oc_diff'].diff()
    ...
    # Preserve the original row index when assigning each stock's rolling correlation.
    data_c['corr_ocdiff_200'] = np.nan
    for _, group in data_c.groupby('ts_code', sort=False):
        data_c.loc[group.index, 'corr_ocdiff_200'] = compute_corr(group).to_numpy()
