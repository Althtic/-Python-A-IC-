...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    try:
        data_c['alpha_61'] = (2 * data_c['scale_rank_vol_multi_rs'] - data_c['scale_rank_ts_argmax_close_10'])
    ...
    return_columns = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'circ_mv', 'industry_name', 'dret', 'raw_factor', factor_name]
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("正在保存数据...")
        save_data(processed_data, "alpha_61.csv")
        logger.info("数据已保存。")
    except Exception as e:
