...
from WQ_Alpha60 import calculate_alpha  # noqa: E402


FACTOR_NAME = "alpha_60"
REQUIRED_COLUMNS = [
    "ts_code",
    "trade_date",
    "open",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Calculate alpha_60 from DolphinDB market tables and sync to factor_daily.")
    ...
    parser.add_argument("--replace", action="store_true", help="Delete existing alpha_60 rows for the factor version before writing.")
    args = parser.parse_args(argv)

    session = connect(args.host, args.port, args.user, args.password)
    run_id = args.run_id or f"alpha_60_{pd.Timestamp.now().strftime('%Y%m%d%H%M%S')}"
