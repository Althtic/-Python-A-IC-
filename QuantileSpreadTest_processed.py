...
def spread_ret_cumsum_calculate(data, layers):
    ...
    # L-S多空收益率序列,用于后续的t检验
    return Spread_ret, t_test_series
# L-S收益率的t检验
...
# 分层回测结果绘图
def plot_multiple_return_metrics(dataframe, layers):
    plt.figure(figsize=(12, 6))
    ax = plt.gca()  # 获取当前的 Axes 对象，用于后续设置刻度
    # 格式转换YYYY-MM-DD
    if not np.issubdtype(dataframe['trade_date'].dtype, np.datetime64):
        # 假设格式是 '20140215' 这样的整数或字符串
    ...
    for q in range(layers):
        ...
        if col_name in dataframe.columns:
            color_idx = q % len(colors)  # 循环使用颜色
            plt.plot(dataframe.index, dataframe[col_name], label=f'group_{q}', color=colors[color_idx], linewidth=1)
        else:
            print(f"警告: 数据中不存在列 '{col_name}'")
    # 单独画多空收益曲线
    excess_col = 'sum_ret_L-S'
    if excess_col in dataframe.columns:
        # 可以指定一种醒目的颜色，比如黑色或红色，或者用新的颜色索引
        plt.plot(dataframe.index, dataframe[excess_col], label='L-S', color='gold', linewidth=2,
                 linestyle='--')
    else:
...
if __name__ == "__main__":
    '''
    params设置(目标因子、分层层数、窗口范围)
    '''
    traget_factor = 'alpha_03'  # 分层多空检验的目标因子名称
    layers = 5 # 分层层数（默认为5）max = 10
    test_window_start = '20200601'
    test_window_end = '20240601'

    '''数据加载与处理'''
    df = data_loading(traget_factor)
    df = data_preprocessing(df,test_window_start,test_window_end)
    '''按日期分组处理,返回后的值需要丢弃多级索引'''
    df_layers_processed = df.groupby('trade_date')[['trade_date','ts_code', traget_factor, 'holding1D_lndret']].apply(
        lambda group: process_group_by_date(group, traget_factor, layers)
    ).reset_index(drop=True)
    # print(df_layers_processed.info())
    '''分层累计收益计算'''
    df_layers_cumulative_ret, spread_ret_series = spread_ret_cumsum_calculate(df_layers_processed, layers)
    # print(spread_ret_series)
    '''多空收益的t-检验'''
    t_test_spread_ret(spread_ret_series)
    '''结果绘图'''
    plot_multiple_return_metrics(df_layers_cumulative_ret, layers)
